...
def train(model, train_loader, val_loader, optimizer, device, epochs=3):
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), './best_bart_model.pth')
...
def evaluate_model(model, val_loader, tokenizer, device):
    ...
    # Aggregate metrics
    metrics = {
        # Use direct access to ROUGE scores
...
def main():
    ...
    val_dataset = dataset['validation'].shuffle(seed=42).select(range(25000))
