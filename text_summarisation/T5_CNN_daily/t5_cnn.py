...
class CNNDailyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        input_text = f"summarize: {self.data[idx]['article']}"
...
def train(model, train_loader, val_loader, optimizer, device, epochs=3):
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), './best_t5_cnn_daily_model.pth')
...
def main():
    ...
    # Load dataset
    # Using the 'train' split for training and 'validation' for validation
    ...
    # Create data loaders
    train_data = CNNDailyDataset(train_dataset, tokenizer)
    val_data = CNNDailyDataset(val_dataset, tokenizer)
